...
function [dataset] = prepare_data_mitdb(window_l, window_t, compute_RR_interval_feature, display_R_peaks)
% Example callings:
% prepare_data_mitdb(18, 180, true, false)
% prepare_data_mitdb(90, 90, true, false)
...
for r = 1:length(records)
    ...
    signal1 = full_signals(:, 2); % Modified Lead II (L II) (Derivacion II)
    signal2 = full_signals(:, 3); % V5
    ...
    for i = 1:length(annotations)
        if((findstr(annotations{i}, ':') > 0))
            ...
            if(sum(strcmp(annotations{i+2}, list_classes)))
                if(pos > window_l && pos < (length(signal1) - window_t))
                    signals{r} = [signals{r} signal1(pos-window_l:pos+ window_t)];
