...
function train_OCSVM_mitdb(window_r_beat, use_RR_interval_features, use_wavelets, norm_features, norm_R)

% train_OCSVM_mitdb(200, true, true, true, true)
...
data_train = [train_N]';
labels_train = [repmat(1, size(train_N, 2), 1)]; 

data_test = [test_N, test_A]';
labels_test = [repmat(1, size(test_N, 2), 1);  (repmat(-1, size(test_A, 2), 1))]; 
    
model_SVM = svmtrain(labels_train, data_train, ['-s 2 -t 2 -n ' num2str(best_C) ' -g ' num2str(best_gamma)]); %, options...
...
save([model_name, '_one_class.mat'], 'model_SVM');
...
disp('Full test');
[predicted_label, accuracy, probs] = svmpredict(labels_test, data_test, model_SVM);
