...
% TODO hearbeat segmentation? QRS onset QRS offset..... not only R peak

function train_SVM_one_against_all(window_l, window_t, morphology_type, use_RR_interval_features, norm_features, norm_R, weight_imbalanced, svm_type, kernel_type)
% train_SVM_one_against_all(80, 80, 0,  true, true, true, true, 0, 2) % Raw Signal
% train_SVM_one_against_all(80, 80, 1,  true, true, true, true, 0, 2) % Wave 
% train_SVM_one_against_all(18, 180, 2, true, true, true, true, 0, 2) % Chazal morphology_weight
% train_SVM_one_against_all(18, 180, 2,  true, true, true, false, 0, 2)% Chazal morphology
%% Includes
% LIBSVM
...
load([full_path, 'data_w_', num2str(window_l), '_', num2str(window_t), '_', list_classes{:}]);
...
for(p = 1:size(mit_data.signals, 2))
    mit_data.signals{p}  = ([mit_data.signals{p}]  - min_Amp) / (max_Amp - min_Amp); % normalize between 0,1
end
...
if(morphology_type == 1 )% wave Signal
    % Wavelets
    features_wave{size(mit_data.signals, 2)} = [];
    
    % Check if they have been already computed
    wavelet_features_filename = [full_path, 'wavelet_feature_w_', num2str(window_l), '_', num2str(window_t), '.mat'];
...
if(morphology_type == 2)% chazal morph 
     % chazal morph 
    features_wave{size(mit_data.signals, 2)} = [];
    % Check if they have been already computed
    wavelet_features_filename = [full_path, 'chzal_morph_feature_w_', num2str(window_l), '_', num2str(window_t), '.mat'];
    ...
    if(exist(wavelet_features_filename, 'file') ~= 2)
        for(s = 1:size(mit_data.signals, 2))
            for(b =1:size(mit_data.signals{s}, 2))
                ...
                i1 = round(1:5.4:54);
                f1 = mit_data.signals{s}(1:54, b);
         
                i2 = round(1:15.75:126);
                f2 = mit_data.signals{s}(72:198, b);
                
                morph = [f1(i1); f2(i2)];
               % morph = ...([mit_data.signals{s}(:, b))
...
model_name = [full_path,  'SVM_models/svm_interpt_w_', num2str(window_l), '_', num2str(window_t)];
