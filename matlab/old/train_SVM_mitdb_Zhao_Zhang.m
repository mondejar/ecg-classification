...
function train_SVM_mitdb_Zhao_Zhang(window_r_beat, use_RR_interval_features, use_wavelets, norm_features, norm_R, svm_type, kernel_type)

% train_SVM_mitdb_Zhao_Zhang(200, true, true, true, true, 0, 2) C-SVM RBF RR  1xx for test, 2xx for training
% train_SVM_mitdb_Zhao_Zhang(200, false, true, true, true, 0, 2) C-SVM RBF 
...
last_index = 1;
patients_subgroup = 'melgani' % 'zhao', 'all', 'melgani'


classes_a = unique(anomaly_subclass);

if(strcmp(patients_subgroup, 'all'))
    N_data_selected = N_data;
    A_data_selected = A_data;
    
else
    if(strcmp(patients_subgroup, 'zhao'))
        % From paper Zhao, Zhang
        patients_used = {'100', '107', '109', '118', '208', '232'};
    elseif(strcmp(patients_subgroup, 'melgani'))
        % From paper Melgani
        patients_used = {'100', '102', '104', '105', '106', '107', '118', '119', '200', '201', '202', '203', '205', '208', '209', '212', '213', '214', '215', '217'};
    end
    
    %% Split data for anomalies
    for(i = 1:length(patients_N))  
        patients_N{i} =  patients_N{i}(length(patients_N{i})-6:length(patients_N{i})-4);
        if( sum(strcmp(patients_N{i},patients_used))) % Add beat
            N_data_selected = [N_data_selected features_N(:, last_index:last_index + ocurrences_N(i) -1)];
    ...
    for(i = 1:length(patients_A))
        ...
        if( sum(strcmp(patients_A{i},patients_used))) % Add beat       
            
            % Add beat to the correct class
            % A_data_selected = [A_data_selected features_A(:, last_index:last_index + ocurrences_A(i) -1)];
            % last_index = last_index + ocurrences_A(i);
            
            beats_patient = last_index:last_index + ocurrences_A(i) -1;
            last_index = last_index + ocurrences_A(i);
            for(b=1:length(beats_patient))
                class_a = find(strcmp(anomaly_subclass(beats_patient(b)), classes_a));
                if(class_a)
                    A_data_selected{class_a} = [A_data_selected{class_a} features_A(:, beats_patient(b))];
                end     
...
% Split data 70 training 30 test (order, random ??)
train_test_ratio = 0.2;

num_N = size(N_data_selected, 2);
N_data_Train = N_data_selected(:, 1:num_N * train_test_ratio);
N_data_Test = N_data_selected(:, (num_N * train_test_ratio)+1:num_N);

% num_A = size(A_data_selected, 2);
% A_data_Train = A_data_selected(:, 1:num_A * train_test_ratio);
% A_data_Test = A_data_selected(:, (num_A * train_test_ratio)+1:num_A);

% For each subclass from A select train_test_ratio
A_data_Train = [];
A_data_Test = [];
for(a=1:length(classes_a))
    num_A = size(A_data_selected{a} , 2);   
    
    A_data_Train = [A_data_Train A_data_selected{a}(:, 1:num_A * train_test_ratio)];
    A_data_Test = [A_data_Test A_data_selected{a}(:, (num_A * train_test_ratio)+1:num_A)];
...
gamma_values = [0.01, 0.05, 0.5]; %[0.5/n_features, 1/n_features, 2/n_features];
if(svm_type == 2) % OCSVM
...
model_name = [model_name, '_', patients_subgroup, '_', train_test_ratio];
model_name
